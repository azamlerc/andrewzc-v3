// Merge entities from a detail list into a main list as props.
// Each detail entity becomes props.<detail-list> on the matching main entity,
// keyed by name. Creates missing main entities if needed.
...
const [mainList, detailList] = process.argv.slice(2).filter(a => !a.startsWith("--"));
const dryRun = process.argv.includes("--dryrun");

if (!mainList || !detailList) {
  console.error("Usage: node merge-list-to-props.js <main-list> <detail-list> [--dryrun]");
  process.exit(1);
}
...
// ---- business logic: compute the value to set on props ----
...
for (const detail of detailEntities) {
  // Parse optional parenthetical: "New York (A Division)" → key=new-york, subName="A Division"
  ...
  if (!mainEntity) {
    if (dryRun) {
      console.log(`Would create: ${mainList}/${baseKey}`);
      continue;
    }
